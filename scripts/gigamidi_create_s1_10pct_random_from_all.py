...
Usage:
    python scripts/gigamidi_create_s1.py [--output data/s1_10pct_random.json]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Create S1: 10% random sample from ENTIRE GigaMIDI"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="data/gigamidi_s1_10pct_random.json",
    ...
    print("=" * 60)
    print("GigaMIDI Subset S1 Creator")
    print("10% Random from EVERYTHING")
    print("=" * 60)
    ...
    # Sort by md5 for reproducibility (deterministic order before random sampling)
    ...
    s1 = random.sample(all_files, k=sample_size)

    print(f"S1 created: {len(s1):,} files")
