#!/usr/bin/env python3
"""
GigaMIDI Subset S1: 10% Random from Everything

This script creates S1 - a random 10% sample from the ENTIRE GigaMIDI dataset
...
S1 Definition (from docs/amt-fine-tuning.md):
- Subset ID: S1
- Description: 10% random from EVERYTHING (all splits)
- Files: ~105k
- Selection Method: Random sampling from all splits, then hash-based train/valid/test split

Usage:
    python scripts/gigamidi_create_s1_10pct_random_from_all.py \
        --output data/gigamidi_s1_10pct_random_from_all.json \
        --sample_size 0.10 \
        --seed 42
...
def collect_from_split(split_name, ds, limit=None):
    """Collect metadata from a single split."""
    files = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Create S1: 10% random sample from ENTIRE GigaMIDI (all splits)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="data/gigamidi_s1_10pct_random_from_all.json",
    ...
    print("=" * 70)
    print("GigaMIDI Subset S1 Creator - 10% Random from EVERYTHING")
    print("=" * 70)
    ...
    s1_sample = random.sample(all_files, k=sample_size)

    sample_time = time.time() - stage_start
    print(f"  Sampled in {sample_time:.1f}s")
    print(f"\n  S1 Subset: {len(s1_sample):,} files selected")
    sys.stdout.flush()
    ...
    train_result = sum(1 for f in s1_sample if f["md5"][0].lower() in train_hashes)
    valid_result = sum(1 for f in s1_sample if f["md5"][0].lower() in valid_hashes)
    test_result = sum(1 for f in s1_sample if f["md5"][0].lower() in test_hashes)

    print(f"\n  After hash-based splitting:")
    print(
        f"    - Train (0-d): {train_result:,} ({100 * train_result / len(s1_sample):.1f}%)"
    )
    print(
        f"    - Valid (e):   {valid_result:,} ({100 * valid_result / len(s1_sample):.1f}%)"
    )
    print(
        f"    - Test (f):    {test_result:,} ({100 * test_result / len(s1_sample):.1f}%)"
    )
    sys.stdout.flush()
    ...
    with open(output_path, "w") as f:
        json.dump(s1_sample, f, indent=2)
    ...
    print(f"  S1 subset size:           {len(s1_sample):,}")
