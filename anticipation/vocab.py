...
# velocity extension: note vocabulary expands by VEL_BINS
# note_with_velocity = NOTE_OFFSET + VEL_BINS * MAX_NOTE + (128*instr + pitch) * VEL_BINS + vel_bin
# Old: REST = NOTE_OFFSET + MAX_NOTE (16512 entries)
# New: REST = NOTE_OFFSET + VEL_BINS * MAX_NOTE (66048 entries)
EVENT_NOTE_TOKENS = VEL_BINS * MAX_NOTE  # 66048
REST = NOTE_OFFSET + EVENT_NOTE_TOKENS

# the control block (mirrors event block with velocity extension)
CONTROL_OFFSET = REST + 1
ATIME_OFFSET = CONTROL_OFFSET + 0
ADUR_OFFSET = ATIME_OFFSET + MAX_TIME
ANOTE_OFFSET = ADUR_OFFSET + MAX_DUR
CONTROL_NOTE_TOKENS = EVENT_NOTE_TOKENS  # 66048

# the special block
SPECIAL_OFFSET = ANOTE_OFFSET + CONTROL_NOTE_TOKENS
