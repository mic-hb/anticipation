...
def safe_logits(logits, idx, curtime=None, allowed_control_pn=None):
    if allowed_control_pn is None:
        ...
        logits[(ANOTE_OFFSET + (instr + 1) * MAX_PITCH) : SPECIAL_OFFSET] = -float(
            "inf"
        )
        logits[vocab["instrument_offset"] :] = -float("inf")
    ...
    if idx % 3 == 0:
        logits[
            vocab["duration_offset"] : vocab["duration_offset"]
            + vocab["config"]["max_duration"]
        ] = -float("inf")
        logits[
            vocab["note_offset"] : vocab["note_offset"] + vocab["config"]["max_note"]
        ] = -float("inf")
    elif idx % 3 == 1:
        logits[
            vocab["time_offset"] : vocab["time_offset"] + vocab["config"]["max_time"]
        ] = -float("inf")
        logits[
            vocab["note_offset"] : vocab["note_offset"] + vocab["config"]["max_note"]
        ] = -float("inf")
...
def construct_prompt(
    instruments, human_instruments, task, tokens, cache, vocab, force_z_cont=False
):
    pad = vocab["pad"]
    ...
    if (len(tokens) + len(z_start) + 1) < 1024:
        ...
        if (len(tokens) + len(z_start) + 1) == 1024 or (
            (len(tokens) + len(z_cont)) % 255 < 3
        ):
            cache = None

        # compute quantized lookback for caching with z_cont
        lookback = max(
            len(tokens) + len(z_cont) - 768 - ((len(tokens) + len(z_cont)) % 255), 0
        )
        prefix = z_cont
...
def add_token(
    ...
    input_ids_and_logits_output_folder="generate_plugin_sim",
):
    assert len(tokens) % 3 == 0

    new_token = []
    input_ids, cache, offset = construct_prompt(
        instruments,
        human_instruments,
        task,
        tokens,
        ...
        for i in range(3):
            if not use_MLC:
                if save_input_ids_and_logits:
                    os.makedirs(
                        f"{input_ids_and_logits_output_folder}/input_ids_and_logits",
                        exist_ok=True,
                    )
                    with open(
                        f"{input_ids_and_logits_output_folder}/input_ids_and_logits/input_ids_{len(tokens)}_{i}.txt",
                        "w",
                    ) as f:
                ...
                if save_input_ids_and_logits:
                    ...
                    with open(
                        f"{input_ids_and_logits_output_folder}/input_ids_and_logits/logits_{len(tokens)}_{i}.txt",
                        "w",
                    ) as f:
...
def generate(
    ...
    use_MLC=False,
    save_input_ids_and_logits=False,
    output_folder="generate_plugin_sim",
):
    if inputs is None:
        inputs = []
    ...
    prompt = ops.pad(
        ops.clip(inputs, 0, start_time, seconds=False, clip_duration=False), start_time
    )
    ...
    future = ops.clip(
        inputs,
        start_time + 1,
        ops.max_time(inputs, seconds=False),
        seconds=False,
        clip_duration=False,
    )
    if debug:
    ...
    chord_controls = ops.clip(
        chord_controls,
        DELTA,
        ops.max_time(chord_controls, seconds=False),
        seconds=False,
        clip_duration=False,
    )
    ...
    tokens, chord_controls, human_controls = ops.anticipate_and_anti_anticipate(
        prompt,
        ops.sort(chord_controls + [CONTROL_OFFSET + token for token in future]),
        human_controls,
    ...
    with tqdm(range(end_time - start_time)) as progress:
        ...
        cache = None
        while True:
            while (current_time >= anticipated_time - chord_delta) or (
                current_time >= anti_anticipated_time - human_delta
            ):
                if (
                    anticipated_time - chord_delta
                    <= anti_anticipated_time - human_delta
                ):
                    # update the cache
                    input_ids, cache, offset = construct_prompt(
                        instruments, human_instruments, task, tokens, cache, vocab
                    )
                    for new_token in [atime - offset, adur, anote]:
                        with torch.no_grad():
                            ...
                            if not use_MLC:
                                if save_input_ids_and_logits:
                                    os.makedirs(
                                        f"{output_folder}/input_ids_and_logits",
                                        exist_ok=True,
                                    )
                                    with open(
                                        f"{output_folder}/input_ids_and_logits/input_ids_{len(tokens)}.txt",
                                        "w",
                                    ) as f:
                                        f.write(str(input_ids.tolist()))
                                input_ids = input_ids.unsqueeze(0).to(model.device)
                                ...
                                if save_input_ids_and_logits:
                                    ...
                                    with open(
                                        f"{output_folder}/input_ids_and_logits/logits_{len(tokens)}_{i}.txt",
                                        "w",
                                    ) as f:
                                        f.write(str(logits.tolist()))
                            else:
                        ...
                        with torch.no_grad():
                            ...
                            if not use_MLC:
                                if save_input_ids_and_logits:
                                    ...
                                    with open(
                                        f"{output_folder}/input_ids_and_logits/input_ids_{len(tokens)}_{i}.txt",
                                        "w",
                                    ) as f:
                                        f.write(str(input_ids.tolist()))
                                input_ids = input_ids.unsqueeze(0).to(model.device)
            ...
            new_token, cache = add_token(
                ...
                use_MLC,
                force_z_cont=force_z_cont,
                save_input_ids_and_logits=save_input_ids_and_logits,
...
def debugchat_forward(dc, input_tokens, kv_caches):
    """
    Parameters
    ----------
    dc : DebugChat
    ...
    if kv_caches is None:
        input_tokens = tvm.nd.array(
            np.array(input_tokens).astype("int32"), device=dc.device
        )
    ...
    with tqdm(range(end_time - start_time)) as progress:
        ...
        while True:
            ...
            new_token, cache = add_token(
                ...
                force_z_cont=force_z_cont,
                save_input_ids_and_logits=save_input_ids_and_logits,
                input_ids_and_logits_output_folder=input_ids_and_logits_output_folder,
            )
            new_time = new_token[0] - TIME_OFFSET
            if new_time >= end_time:
                break
...
def construct_prompt_no_cache(
    instruments, human_instruments, task, tokens, vocab, force_z_cont=False
):
    pad = vocab["pad"]
...
def add_token_no_cache(
    ...
    for i in range(3):
        ...
        input_ids, offset = construct_prompt_no_cache(
            instruments,
            human_instruments,
            task,
            current_tokens,
...
def generate(
    ...
    # prompt is events up to start_time
    prompt = ops.pad(
        ops.clip(inputs, 0, start_time, clip_duration=False, seconds=False), start_time
    )
    ...
    controls = ops.clip(
        controls,
        DELTA,
        ops.max_time(controls, seconds=False),
        clip_duration=False,
        seconds=False,
    )
    ...
    tokens, controls = ops.anticipate(
        prompt, ops.sort(controls + [CONTROL_OFFSET + token for token in future])
    )
    ...
    with tqdm(range(end_time - start_time)) as progress:
        ...
        while True:
            ...
            new_token = add_token(
                model, z, tokens, top_p, max(start_time, current_time)
            )
            new_time = new_token[0] - TIME_OFFSET
            if new_time >= end_time:
                break
