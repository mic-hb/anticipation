...
def prepare_local_midi_autoregress(
    midifile, tripletmidivocab, localmidivocab, task, transcript
):
    with open(midifile, "r") as f:
        events, truncations, status = tokenize.maybe_tokenize(
            [int(token) for token in f.read().split()]
        )
    ...
    for time, dur, note in zip(events[0::3], events[1::3], events[2::3]):
        if (
            time != tripletmidivocab["separator"]
            and dur != tripletmidivocab["separator"]
            and note != tripletmidivocab["separator"]
        ):  # separator in convert.py is pulled from vocab.py but it has same value as tripletmidivocab's separator (i.e. 55025) --> skip any of the separators
            ...
            tokens.append(
                dur
                - tripletmidivocab["duration_offset"]
                + localmidivocab["duration_offset"]
            )
            # print(f"dur: {dur}")
            tokens.append(
                note - tripletmidivocab["note_offset"] + localmidivocab["note_offset"]
            )
            # print(f"note: {note}")
...
def prepare_local_midi_old(
    midifile, tripletmidivocab, localmidivocab, task, transcript
):  # only works for anticipation right now
    with open(midifile, "r") as f:
        events, truncations, status = tokenize.maybe_tokenize(
            [int(token) for token in f.read().split()]
        )
    ...
    end_time = ops.max_time(
        events, seconds=False
    )  # NOTE: do we need the same thing here
    ...
    if task == "autoregress":
        ...
        if task == "instrument":
            z = [
                localmidivocab["control_end"]
            ]  # we want new token that is task_instr and control_end --> replace delta offset and say task_instr is global control offset + 3
            instruments = list(ops.get_instruments(events).keys())
            if len(instruments) < 2:
                # need at least two instruments for instrument anticipation
                return events, 4  # status 4 == too few instruments
    ...
    anticipated_events, controls = ops.anticipate(
        events, controls
    )  # assuming controls completely consumed as well

    # separators are in groups of three here, strip them out
    # fix the note durations by subtracting offset and then adding the note offset
    for time, dur, note in zip(
        anticipated_events[0::3], anticipated_events[1::3], anticipated_events[2::3]
    ):
        if (
            time != tripletmidivocab["separator"]
            and dur != tripletmidivocab["separator"]
            ...
            if note == localmidivocab["tick"]:
                ...
                if (
                    NOTE_OFFSET <= note < CONTROL_OFFSET
                ):  # event, values from vocab.py --> verify if same as tripletmidi
                    tokens.append(
                        time
                        - recent_tick
                        - tripletmidivocab["time_offset"]
                        + localmidivocab["time_offset"]
                    )
                    tokens.append(
                        dur
                        - tripletmidivocab["duration_offset"]
                    ...
                    tokens.append(
                        note
                        - tripletmidivocab["note_offset"]
                        + localmidivocab["note_offset"]
                    )
                elif (
                    CONTROL_OFFSET <= note < SPECIAL_OFFSET
                ):  # control, values from vocab.py --> verify if same as tripletmidi or is it anote_offset <= note < special_offset
                    tokens.append(
                        time
                        - recent_tick
                        - tripletmidivocab["atime_offset"]
                        + localmidivocab["atime_offset"]
                    )
                    tokens.append(
                        dur
                        - tripletmidivocab["aduration_offset"]
                        + localmidivocab["aduration_offset"]
                    )
                    tokens.append(
                        note
                        - tripletmidivocab["anote_offset"]
                        + localmidivocab["anote_offset"]
                    )
                else:
...
def prepare_local_midi(
    midifile, tripletmidivocab, localmidivocab, task, transcript, delta_seconds=None
):
    with open(midifile, "r") as f:
        events, truncations, status = tokenize.maybe_tokenize(
            [int(token) for token in f.read().split()]
    ...
    if task == "autoregress":
        ...
        if task == "instrument":
            ...
            if (
                delta_seconds is None
            ):  # TODO: check if this is fine for controllable anticipation interval
    ...
    if delta_seconds is None:
        anticipated_events, remaining_controls = ops.anticipate(
            events_with_ticks, controls
        )
    else:
        anticipated_events, remaining_controls = ops.anticipate(
            events_with_ticks, controls, delta=delta_seconds * time_res
        )
    ...
    for time, dur, note in zip(
        ...
        if (
            time == tripletmidivocab["separator"]
            or dur == tripletmidivocab["separator"]
            or note == tripletmidivocab["separator"]
        ):
            continue
        ...
        if note == localmidivocab["tick"]:
            ...
            if (
                tripletmidivocab["note_offset"]
                <= note
                < tripletmidivocab["control_offset"]
            ):  # regular event token
                ...
                tripletmidivocab["anote_offset"]
                <= note
                < tripletmidivocab["special_offset"]
            ):  # Control
...
def prepare_triplet_midi(midifile, vocab, task, transcript):
    ...
    if task == "autoregress":
        ...
        if task == "instrument":
            ...
            events, controls = tokenize.extract_spans(events, 0.05)
        elif task == "random":
...
def pack_tokens(sequences, output, idx, prepare, factor, config, seqlen, vocab):
    ...
    stats = 5 * [
        0
    ]  # (short, long, too many instruments, too few instruments, inexpressible)
    with open(output, "w") as outfile:
        ...
        for sequence in tqdm(sequences, desc=f"#{idx}", position=idx + 1, leave=True):
            ...
            for _ in range(factor):
                ...
                concatenated_tokens.extend(
                    separator + z + tokens
                )  # this is entirely new sequence being concatenated to list, so new z
                z_tokens.extend([z] * len(separator + z + tokens))

                z = z_tokens[
                    0
                ]  # need to redefine z to be the correct z, not just most recent
...
def main(args):
    ...
    LAKH_TRAIN = [x for x in LAKH_SPLITS if x not in ('e', 'f')]
    LAKH_VALID = ['e']
    LAKH_TEST = ['f']

    all_splits = [("train", LAKH_TRAIN), ("valid", LAKH_VALID), ("test", LAKH_TEST)]

    for split_name, split_dirs in all_splits:
        if args.vocab == "triplet-midi":
            from anticipation.vocabs.tripletmidi import vocab as tripletmidivocab

            prepare = partial(
                prepare_triplet_midi,
                vocab=tripletmidivocab,
                task=args.task,
            ...
            prepare = partial(
                prepare_local_midi,
                tripletmidivocab=tripletmidivocab,
                localmidivocab=localmidivocab,
                task=args.task,
                transcript=args.transcript,
        ...
        if args.task == "autoregress":
            assert args.factor == 1, (
                f"Autoregressive preprocessing has no randomness; cannot apply augmentation factor {factor}"
            )

        files = []
        for split_dir in split_dirs:
            files += list(glob(os.path.join(args.datadir, f"{split_dir}/*.compound.txt"), recursive=True))
        ...
        shards = [
            files[i * n : (i + 1) * n] for i in range(args.workers)
        ]  # dropping a few tracks (< args.workers)
        outfiles = os.path.join(
            args.outdir, os.path.basename(args.datadir) + "." + split_name + ".{t}.shard-{s:03}.txt"
        )
        ...
        if args.debug:
            ...
            with Pool(
                processes=args.workers, initargs=(RLock(),), initializer=init_worker
            ) as pool:
                results = pool.starmap(
                    ...
                    zip(
                        ...
                        config,
                        context,
                        args.workers * [chosen_vocab],
                    ),
                )

        seqcount, too_short, too_long, many_instr, few_instr, discarded_seqs = (
        ...
        print(
            f"  => Discarded {too_short + too_long + many_instr + few_instr} event sequences"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "context", type=int, help="context length for packing training sequences"
    )
    parser.add_argument(
        "-v",
        "--vocab",
        default="triplet-midi",
        help="name of vocabulary to use for tokenization",
    )
    parser.add_argument(
        "-f", "--factor", type=int, default=1, help="augmentation factor"
    )
    parser.add_argument(
        "-t", "--transcript", action="store_true", help="transcribed midi file"
    )
    parser.add_argument(
        "--workers", type=int, default=16, help="number of workers/shards"
    )
    parser.add_argument(
        "--debug", action="store_true", help="debugging (single shard; non-parallel)"
    )
    parser.add_argument(
        "--delta",
        type=int,
        default=None,
        help="used for controllable anticipation interval, in case using a different anticipation interval",
    )
